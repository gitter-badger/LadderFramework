...
import org.ladderframework.Status.OK
...
class HttpRequestSpec extends FunSpec with GivenWhenThen {
  ...
  val dummyResponse = new HttpResponse{
  	...
  	def httpResponse()(implicit context: Context): Future[HttpStringResponseOutput] = 
  ...
  describe("Matching HttpRequest") {
  	...
  	it("should match on path and json content"){
  		assert(HttpReq(GET, "path" :: Nil, SessionId("sessionID"), Map("Accept" -> "application/json")) match {
  ...
  object HttpReq{
  	def apply(givenMethod: Method, givenPath: List[String], givenSession: SessionId = SessionId(""), givenHeaders: Map[String, String] = Map()) = new HttpRequest{
  		...
  		override def sessionId = givenSession
			...
			override def parts: List[Part] = Nil 
			override def invalidateSession(): Unit = {}
  	}
  }
