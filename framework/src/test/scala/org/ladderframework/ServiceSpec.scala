...
class ServiceSpec(system: ActorSystem) extends TestKit(system) with WordSpecLike with GivenWhenThen with BeforeAndAfterAll with ScalaFutures{
	...
	"The framework" when {
		...
		"recieving a HTTP GET request with parameter to an in memory response" should {
			val request = httpRequest(GET, sessionID, "hello" :: "world" :: Nil, Map("parameter" -> List("postedValue")))
...
//				assert(asyncRequestHandler.latch.getCount === 0)
		...
		"recieving a HTTP POST request to an in memory response (Hello World time)" should {
			val request = httpRequest(POST, sessionID,  "hello" :: "world" :: Nil, Map("parameter" -> List("postedValue")))
		...
		"handle statefull response" should {
			"handle POST REDIRECT GET" in {
				...
				val httpServletResponseStatefull = call(httpRequest(POST, sessionID,  "post" :: stateful :: func :: Nil, Map("key" -> List("value")))).futureValue
				assert(httpServletResponseStatefull.status === Found)
				val location = httpServletResponseStatefull.headers.collect{case Location(l) => l}.head
				assert(location.path.toString.startsWith(List("some", "where", "new").mkString("/", "/", "")))
				
				val param = location.query
				val httpServletResponseRedirect = call( httpRequest(GET, sessionID, List("some", "where", "new"), Map(param.key -> List(param.value)))).futureValue
			...
			"handle simple ajax post" in {
				...
				val httpServletResponseAjax = call( httpRequest(POST, sessionID, "ajax" :: statefull :: id :: Nil, Map("key" -> List("value")))).futureValue
			...
			"handle simple ajax callback" in {
				...
				val httpServletResponseAjax = call(httpRequest(POST, sessionID,  "ajax" :: statefull :: Nil, Map(key -> List("inputValue")))).futureValue
				assert(httpServletResponseAjax.status === OK)
				assert(httpServletResponseAjax.contentType === ContentType.`text/javascript`)
			...
			"handle simple ajax callback handler" in {
				...
				val httpServletResponseAjax = call(httpRequest(POST, sessionID,  "ajax" :: statefull :: key :: Nil, Map(key -> List("inputValue")))).futureValue
				assert(httpServletResponseAjax.status === OK)
